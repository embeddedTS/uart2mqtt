...
class UART2MQTT:
    ...
    def mqtt_connect(self) -> None:
        ...
        def on_message(
            ...
            port_name = message.topic.split("/")[-2]  # Extract port name
            if port_name in self.serial_ports:
                ...
                logger.warning(
                    "Port %s not found for topic %s", port_name, message.topic
                )
    ...
    def monitor_serial_ports(self) -> None:
        while not self.stop_event.is_set():
            try:
                ...
                # udev is going through a renaming process where they have 2
                # namespaces for usb.  Without this, we add the same devices
                # multiple times.
                ...
                available_ports = set(filtered_ports)
                current_ports = set(self.serial_ports.keys())
                new_ports = available_ports - current_ports - self.ignored_ports
                ...
                for port in new_ports:
                    ...
                    if real_dev_path in self.opened_real_devices:
                        logger.debug(
                            f"Skipping duplicate symlink {port} → {real_dev_path}"
                        )
                    ...
                    if vid_pid:
                        ...
                        if usb_allowed(vid, pid, self.cfg.usb_match):
                            logger.info(
                                f"Opening Serial Port <{vid}:{pid}>({real_dev_path}): {port}"
                            )
                        ...
                        logger.warning(
                            f"Could not resolve VID:PID for {port}, ignoring"
                        )
                        self.ignored_ports.add(port)
                ...
                if e.filename == SERIAL_BASE_PATH:
                    logger.warning(
                        f"{SERIAL_BASE_PATH} not found, retrying in 1 second"
                    )
                    time.sleep(1)
                else:
    ...
    def handle_serial(
        self, port: str, serial_conn: Serial, serial_output_topic: str
    ) -> None:
        try:
            while not self.stop_event.is_set():
            ...
            self.serial_ports.pop(port, None)
            real_port = serial_conn.port
            if real_port is not None:
                real_dev = os.path.realpath(real_port)
    ...
    def run(self) -> None:
        ...
        try:
            monitor_thread = threading.Thread(
                target=self.monitor_serial_ports, daemon=True
            )
